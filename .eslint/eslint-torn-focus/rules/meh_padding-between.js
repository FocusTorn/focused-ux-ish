	...
	create(context) {
		...
		function checkPadding(prevNode, nextNode) { //>
			...
			for (const currNodeType in options) {
				...
				const key = `${prevNodeType}-${nextNodeType}` // Create a unique key for the pair

				if (checkedPairs.has(key)) { continue } // Skip if already checked
				checkedPairs.add(key) // Mark as checked

				let expectedLines

				// Prioritize "next" rules for consistency
				if (options[prevNodeType]?.next?.[nextNodeType] !== undefined) {
					expectedLines = options[prevNodeType].next[nextNodeType]
				}
				else if (options[nextNodeType]?.prev?.[prevNodeType] !== undefined) {
					expectedLines = options[nextNodeType].prev[prevNodeType]
				}
				else if (options[prevNodeType]?.next?.['*'] !== undefined) {
					expectedLines = options[prevNodeType].next['*']
				}
				else if (options[nextNodeType]?.prev?.['*'] !== undefined) {
					expectedLines = options[nextNodeType].prev['*']
				}

				if (expectedLines !== undefined) {
					verifySpacing(context, prevNode, nextNode, expectedLines, 'next') // Consistent target for error message
				}
			}
		} //<
		...
		function findPrevNode(node, sourceCode) { //>
			...
			let prev = ASTUtils.getLeftSibling(node)

			while (prev && isPaddingComment(prev, sourceCode, context)) {
				prev = ASTUtils.getLeftSibling(prev)
			}
		...
		function _verifyRule(context, options, prevNode, nextNode, currNodeType, target) { //>
