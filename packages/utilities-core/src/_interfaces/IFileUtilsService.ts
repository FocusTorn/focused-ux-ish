...
export interface IFileUtilsService { //>
	...
	iFspWriteFile: (path: import('node:fs').PathLike | import('node:fs/promises').FileHandle, data: string | Uint8Array, options?: import('node:fs').WriteFileOptions) => Promise<void>
	iFspAccess: (path: import('node:fs').PathLike, mode?: number) => Promise<void>
	iFspMkdir: (path: import('node:fs').PathLike, options?: import('node:fs').MakeDirectoryOptions) => Promise<string | undefined>
