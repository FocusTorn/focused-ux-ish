...
export class PathUtilsService implements IPathUtilsService { //>
	...
	public getNormalizedDirectory( //>
		...
		try {
			...
			this.iCommonUtils.errMsg(`Error in getNormalizedDirectory for ${filePath}. Falling back to dirname.`, error)
			return this.nodePathDirname(normalPath)
		}
	} //<
	...
	public getDottedPath( //>
		...
		try {
			...
			const normalizedTargetPathForStat = this.nodePathIsAbsolute(targetPath) ? targetPath : this.nodePathResolve(targetPath)
			...
			this.iCommonUtils.errMsg(`Error in getDottedPath for target '${targetPath}' from '${pointingPath}'`, error)
			return null
		}
	} //<

	public iPathJoin(...paths: string[]): string { //>
