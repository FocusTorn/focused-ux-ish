...
export async function validateKnip() { //>
		...
		if (knipStdout.trim()) {
			...
			if (typeof parsedOutput === 'object' && parsedOutput !== null) {
				...
				errorMessages += `${chalk.red(
					'Knip JSON output was not in the expected object format. Output:\n',
				)}${knipStdout.trim()}\n`
			...
			errorMessages += chalk.red(
				'Knip command may have failed without producing JSON output, despite --no-exit-code.\n',
			)
			return { success: false, errorMessages: errorMessages.trim(), issueCount: issueCount || 1 }
		}
	} catch (parseError) {
	...
	knipReport.issues.forEach((issueInFile) => {
		...
		Object.keys(issueInFile).forEach((categoryKey) => {
			...
			} else if (categoryKey === 'enumMembers' && typeof itemsInCategory === 'object' && itemsInCategory !== null && Object.keys(
				itemsInCategory,
			).length > 0) {
		...
		if (totalOtherIssuesCount > 0) {
			Object.entries(issuesByTypeGlobal).forEach(([categoryKey, items]) => {
				if (items.length > 0) {
					if (firstSectionPrinted)
						detailOutput += '\n'
					let title = knipCategoryTitles[categoryKey] || categoryKey.replace(/([A-Z])/g, ' $1').replace(
						/^./,
						str => str.toUpperCase(),
					)
					...
					items.forEach((itemDetail) => {
						const name = itemDetail.name || (typeof itemDetail === 'string'
							? itemDetail
